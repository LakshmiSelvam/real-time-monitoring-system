...
import AddFactory from "../../HomeLayout/Component/FactoryManage/AddFactoryDetails";
...
const LoginForm = () => {
  ...
  var [ErrorUserName, setErrorUsername] = useState(false);
  var [ErrorPassword, setErrorPassword] = useState(false);
  var [ErrorCompanycode, setErrorCompanycode] = useState(false);
  var [logError, setLogError] = useState(false);
  const [showpassword, setShowpassword] = useState(false);

  useEffect(() => {
    if (localStorage.authToken !== undefined && localStorage.ack == "OK") {
  ...
  const loginFormButtonHandler = async (event) => {
    ...
    if (validator.isEmail(loginForm.username) === false) {
    ...
    if (loginForm.companycode.length === 0) {
      setErrorMessage("Please enter a company code");
      return setErrorCompanycode(true);
    }

    // Validate the password using regex
    if (!passwordRegex.test(loginForm.password)) {
      setErrorMessage(
        "Password: 5-8 chars, 1 lowercase, 1 uppercase, 1 digit."
      );
      return setErrorPassword(true);
    }
    ...
    if (
      ...
      let body = {
        ...
        companyCode: loginForm.companycode,
      };
      ...
      if (result.fetchStatus === "failure") {
        ...
        if (result.result.message === "success") {
          localStorage.setItem("authToken", result.result.data.authToken);
          localStorage.setItem("refToken", result.result.data.refreshToken);
          // navigate("/home/dashboard");
          // fetchUser(result.result.data.authToken);
          dispatch(setLoading(false));
          setShowPopup(true);
        } else {
      ...
      <div className="right_split_body">
        ...
        <div className="right_middle_body">
          ...
          <div className="input-container">
            ...
            <span data-testid="username-error-field" className="error-message">
              {ErrorUserName && ErrorMessage}
            </span>

            <div className="companycode-container">
              <InputField
                label="Company Code"
                // autoComplete="off"
                type="text"
            ...
            <span data-testid="password-error-field" className="error-message">
              {ErrorPassword && ErrorMessage}
              {logError && ErrorMessage}
            </span>
          </div>
          <div className="forgot-password" onClick={forgotButtonHandler}>
            <div>I forgot my Password</div>
          </div>

          <button className="login-button" onClick={loginFormButtonHandler}>
            Login
          </button>
          <div className="login-message">
            {showPopup && (
              <AddFactory message="Login Successful!" login={true} />
            )}
          </div>
        </div>
